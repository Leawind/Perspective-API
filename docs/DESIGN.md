...
> - 状态标注：✅ 已实现且当前生效 / 🚧 部分实现或进行中 / 📋 仅为规划目标
> - 标注为 📋 或 🚧 的节描述目标而非事实，不得当作既成实现引用
> - 本文代表目标态：实现变更前先更新本文，不允许滞后于实现
...
服务发现的同 ID 冲突由声明的 precedence 决定，高者胜出，平局按行为类名的字典序；`order` 仅用于显示排序。运行时注册遇到重复 ID 直接失败，不参与冲突解决。
...
Perspective API 持有并持久化唯一的玩家 Perspective 选择。内置 Perspective Switcher 只负责玩家交互，不拥有选择状态。它与自定义条件逻辑都直接通过同一个 `PerspectiveSelection` 读取和写入当前选择，两种选择来源地位相同。

该选择只保存原始 Perspective ID，不保存解析后的回退结果。ID 实际变化后，选择对象完成状态修改，再同步通知已注册的监听器；相同 ID 不触发事件。监听器在事件中产生的后续选择变化会排队到当前通知结束后依次发布。这样，其他功能可以在自己的上下文对象与 API 选择之间同步状态，而不需要与具体的玩家交互方式耦合。
...
- 覆盖注册携带稳定的诊断 ID，用于在日志中归因 supplier 失败

## 内置切换器的禁用模式 ✅

内置 Perspective Switcher 的轮盘界面可以整体禁用，并且默认禁用。该开关作为玩家设置持久化，与配置界面入口无关：配置界面始终可以打开，并作为重新启用轮盘的途径。

禁用时：

- 视角切换按键在按下事件直接循环切换，循环对象为所有声明了 `switchable` 且当前可用的 Perspective，顺序与轮盘候选项一致
- 视角切换按键没有长按行为
- 轮盘界面及其编辑模式无法打开

禁用只影响内置切换器的交互方式，不影响 `PerspectiveSelection` 本身：其他功能仍可通过 API 修改选择，切换过渡照常生效。启用后，轻点循环、长按打开轮盘和编辑模式的行为恢复。
...
切换过渡机制可以整体禁用，并且默认禁用。该开关作为玩家设置持久化，禁用期间过渡时长和 Perspective 的过渡许可均不生效：切换时直接采用新 Perspective 的目标状态，已在进行中的过渡在禁用后立即停止插值。启用后按本节其余规则执行过渡。
...
原版存在两条独立的 FOV 计算路径：世界渲染的 FOV 基于玩家的 FOV 设置和 FOV 效果修饰；手部（HUD）渲染的 FOV 不使用玩家设置，而是基于固定值（约 70 度）。旧版本中原版以同一个方法配合布尔参数区分两条路径，26.1 起拆分为独立方法，26.3 起手部路径的 FOV 进一步成为独立的相机状态，两条路径不再共享任何计算入口。
