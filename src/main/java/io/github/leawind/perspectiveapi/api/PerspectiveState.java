...
///
/// Comprises position (world space), rotation (API convention, +Z forward), and projection
/// settings. The method or callback providing a state documents whether it is temporary or can be
/// retained.
...
public interface PerspectiveState {
  ...
  /// the state pipeline consistent across supported Minecraft versions. It is always finite and in
  /// the open range `(0, 180)`.
  float getFovDeg();
