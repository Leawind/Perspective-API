...
/// A lightweight event emitter for subscribing to and emitting events of type `E`.
///
...
public interface SimpleEventEmitter<E> {
  ...
  ///
  /// @param <E> the event type
  /// @return a new {@link Owned} emitter instance
  ...
  interface Owned<E> extends SimpleEventEmitter<E> {
    ...
    /// Emits an event to all registered listeners with a `null` event object.
    void emit();
